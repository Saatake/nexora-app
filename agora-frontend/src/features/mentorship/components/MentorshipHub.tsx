...
type MentorshipHubProps = {
  ...
  onCompleteMentorship: (mentorshipId: number) => Promise<{ success: boolean; error?: string }>;
...
export const MentorshipHub = ({
  ...
  const handleComplete = async () => {
    ...
    setIsProcessing(true);
    setActionError('');
    const res = await onCompleteMentorship(mentorship.id);
    setIsProcessing(false);
    setShowCompleteConfirm(false);
    if (!res.success) setActionError(res.error || 'Erro ao concluir mentoria.');
  };
    ...
    return (
      <div className="rounded-2xl border border-[var(--agora-border)] bg-[var(--agora-panel)] p-8 shadow-[var(--agora-shadow)] text-center">
        ...
        <p className="text-xs text-[var(--agora-muted)] max-w-md mx-auto mb-6">
          A mentoria conecta o projeto a um professor especialista para acompanhar entregas, revisar marcos e oferecer suporte contínuo via chat privativo.
        </p>
  ...
  // CASO 3: MENTORIA CONCLUÍDA COM SUCESSO
  ...
  return (
    <div className="space-y-6">
      {/* Banner Superior */}
      {isCompleted ? (
        <div className="rounded-2xl border border-emerald-500/30 bg-gradient-to-r from-emerald-500/10 via-[var(--agora-panel)] to-[var(--agora-panel)] p-6 shadow-[var(--agora-shadow)]">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-2xl bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 flex items-center justify-center flex-shrink-0 shadow-sm border border-emerald-500/20">
                <Award size={32} />
              </div>
              <div>
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  <h3 className="font-bold text-base text-[var(--agora-ink)]">
                    Mentoria Concluída com Sucesso! 🎓
                  </h3>
                  <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-emerald-500/20 text-emerald-700 dark:text-emerald-300">
                    Projeto Orientado & Finalizado
                  </span>
                </div>
                <p className="text-xs text-[var(--agora-muted)] leading-relaxed">
                  Orientador responsável: <strong className="text-[var(--agora-ink)]">Prof. {mentorship.professorName}</strong> ({mentorship.professorCourse || 'Docente'}). Concluída com êxito em {formatSinceDate(mentorship.endedAt)}.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 rounded-2xl border border-[var(--agora-border)] bg-[var(--agora-panel)] shadow-[var(--agora-shadow)]">
          ...
          {/* Ações do Orientador / Equipe */}
          <div className="flex items-center gap-2 self-start sm:self-auto">
            {isAssignedMentor && (
              <button
                onClick={() => setShowCompleteConfirm(true)}
                className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[#0a5c2f] hover:bg-[#084925] text-white text-xs font-semibold shadow-sm transition-all"
              >
            ...
            {(isTeamMember || isAssignedMentor) && (
              <button
                ...
                className="text-xs text-rose-600 hover:text-rose-700 hover:underline flex items-center gap-1 font-semibold p-1"
              >
                Encerrar
              </button>
            )}
      ...
      {/* Navegação entre Abas: Marcos vs Chat */}
      ...
      {activeTab === 'goals' ? (
        <MentorshipGoalsSection
          ...
          isProfessor={isAssignedMentor && !isCompleted}
          isStudentMember={isTeamMember && !isCompleted}
      ...
      {/* Modal Confirmação: Concluir Mentoria */}
      {showCompleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="w-full max-w-md rounded-2xl border border-[var(--agora-border)] bg-[var(--agora-panel)] p-6 shadow-2xl">
            <div className="flex items-center gap-2.5 text-[#0a5c2f] mb-3">
              <Award size={24} />
              <h3 className="font-bold text-base text-[var(--agora-ink)]">Concluir Mentoria Acadêmica?</h3>
            </div>
            <p className="text-xs text-[var(--agora-muted)] leading-relaxed mb-5">
              Ao concluir a mentoria, você oficializa que este projeto acadêmico atingiu seus objetivos pedagógicos e foi orientado e aprovado por você. O projeto manterá o selo de orientado e os alunos receberão uma notificação de parabéns.
            </p>
            <div className="flex items-center justify-end gap-2">
              ...
              <button
                ...
                className="px-4 py-2 text-xs font-semibold rounded-xl bg-[#0a5c2f] hover:bg-[#084925] text-white transition-colors flex items-center gap-1.5"
              >
      ...
      {/* Modal Confirmação de Encerramento (Revogação) */}
      {showRevokeConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="w-full max-w-md rounded-2xl border border-[var(--agora-border)] bg-[var(--agora-panel)] p-6 shadow-2xl">
            ...
            <p className="text-xs text-[var(--agora-muted)] leading-relaxed mb-5">
              Ao encerrar sem concluir, o vínculo formal de orientação deste projeto será finalizado e revogado.
            </p>
            <div className="flex items-center justify-end gap-2">
              <button
                type="button"
